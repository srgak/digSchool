...
export class TooltipDirective implements OnInit, OnDestroy {
  ...
  @HostListener('mouseover') private onMouseIn(): void {
    ...
    this.r2.setStyle(
      this.tooltipBlock,
      'top',
      `${tooltipPosition.top - tooltipPosition.height - 10}px`,
    );
    this.r2.setStyle(
      this.tooltipBlock,
      'left',
      `${tooltipPosition.left + tooltipPosition.width / 2 - this.tooltipBlock.clientWidth / 2}px`,
    );
  }
  @HostListener('mouseleave') private onMouseLeave(): void {
    this.tooltipBlock.remove();
  }
  private readonly element: Element = this.elRef.nativeElement;
  private readonly tooltipBlock: Element;
  constructor(
